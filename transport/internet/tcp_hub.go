...
func RegisterTransportListener(protocol string, listener ListenFunc) error {
	if _, found := transportListenerCache[protocol]; found {
		return fmt.Errorf("listener already registered: %s.", protocol)
	}
...
func ListenUnix(ctx context.Context, address net.Address, settings *MemoryStreamConfig, handler ConnHandler) (Listener, error) {
	if settings == nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to create default unix stream settings, err: %v", err)
		}
		settings = s
	}
	...
	if listenFunc == nil {
		return nil, fmt.Errorf("%s unix istener not registered.", protocol)
	}
	listener, err := listenFunc(ctx, address, net.Port(0), settings, handler)
	if err != nil {
		return nil, fmt.Errorf("failed to listen on unix address: %s", address)
	}
	return listener, nil
}
...
func ListenTCP(ctx context.Context, address net.Address, port net.Port, settings *MemoryStreamConfig, handler ConnHandler) (Listener, error) {
	if settings == nil {
		s, err := ToMemoryStreamConfig(nil)
		if err != nil {
			return nil, fmt.Errorf("failed to create default stream settings, err: %v", err)
		}
		settings = s
	}
	...
	if address.Family().IsDomain() {
		return nil, fmt.Errorf("domain address is not allowed for listening: %s", address.Domain())
	}

	protocol := settings.ProtocolName
	listenFunc := transportListenerCache[protocol]
	if listenFunc == nil {
		return nil, fmt.Errorf("%s listener not registered.", protocol)
	}
	listener, err := listenFunc(ctx, address, port, settings, handler)
	if err != nil {
		return nil, fmt.Errorf("failed to listen on address: %s:%d", address, port)
	}
	return listener, nil
}
