		...
		return c.Control(func(fd uintptr) {
			for _, controller := range controllers {
				if err := controller(network, address, c); err != nil {
					// todo session.ExportIDToError(ctx)
					log.Printf("failed to apply external controller, err: %v", err)
				}
			}
			...
			if sockopt != nil {
				if err := applyInboundSocketOptions(network, fd, sockopt); err != nil {
					// todo  session.ExportIDToError(ctx)
					log.Printf("failed to apply socket options to incoming connection, err: %v", err)
				}
			}
	...
	case *net.UnixAddr:
		...
		if (runtime.GOOS == "linux" || runtime.GOOS == "android") && address[0] == '@' {
			...
			if s := strings.Split(address, ","); len(s) == 2 {
				...
				if perr != nil {
					return nil, fmt.Errorf("failed to parse permission: %s, err: %v", s[1], perr)
				}
			...
			callback = func(l net.Listener, err error) (net.Listener, error) {
				...
				if err != nil {
					l.Close()
					return nil, fmt.Errorf("failed to set permission for %s, err: %v", address, err)
				}
				return l, nil
			}
		}
